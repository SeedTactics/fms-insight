  ...
  public static class ParseIso8601Duration
  {
    public static bool TryParse(string duration, out TimeSpan t)
    {
      ...
      for (int i = 1; i < duration.Length; i++)
      {
        ...
        if (!TryParseNumber(duration, ref i, out decimal num)) return false;
    ...
    private static bool TryParseNumber(string s, ref int idx, out decimal n)
    {
      ...
      if (hasDecimal)
      {
        var txt = s.Substring(startIdx, idx - startIdx).Replace(",", CultureInfo.InvariantCulture.NumberFormat.NumberDecimalSeparator);
      ...
      {
        if (decimal.TryParse(s.Substring(startIdx, idx - startIdx), NumberStyles.None, CultureInfo.InvariantCulture, out n))
        {
          return true;
        }
      }
