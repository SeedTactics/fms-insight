...
export class JobsClient {
    ...
    removeMaterialFromQueue(materialID: number): Promise<void> {
        let url_ = this.baseUrl + "/api/v1/jobs/material/{materialID}/queue";
        if (materialID === undefined || materialID === null)
            throw new Error("The parameter 'materialID' must be defined.");
        url_ = url_.replace("{materialID}", encodeURIComponent("" + materialID));
...
export class CurrentStatus implements ICurrentStatus {
    ...
    init(data?: any) {
        if (data) {
            ...
            if (data["_queues"]) {
                this._queues = {};
                for (let key in data["_queues"]) {
                    if (data["_queues"].hasOwnProperty(key))
                        this._queues[key] = data["_queues"][key] ? QueueSize.fromJS(data["_queues"][key]) : new QueueSize();
                }
            }
            this.latestScheduleId = data["LatestScheduleId"];
        }
    ...
    toJSON(data?: any) {
        ...
        if (this._queues) {
            data["_queues"] = {};
            for (let key in this._queues) {
                if (this._queues.hasOwnProperty(key))
                    data["_queues"][key] = this._queues[key];
            }
        }
        data["LatestScheduleId"] = this.latestScheduleId;
        return data;
