...
- Immutable data structures everywhere: `System.Collections.Immutable` in C#,
  `@seedtactics/immutable-collections` in TypeScript
- Functional programming: avoid side effects and OOP
- Strict types: never use TypeScript `any`; use `readonly` properties and
  `ReadonlyArray`/`ReadonlyMap`. Complex dependent-style types are fine if needed.
